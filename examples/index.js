window.addEventListener("load", function () {
  ...
  var wavyPlaneGeom = function () {
    ...
    for (u = 0; u < numPoints; u++) {
      ...
      for (v = 0; v < numPoints; v++) {
        ptsV.push([u / numPoints - 0.5, Math.random() - 0.5, v / numPoints - 0.1]);
      }
      pts.push(ptsV);
    }
  ...
  m.setPosition(new THREE.Vector3(0, -0.8, 0));
  geom.applyMatrix4(m);
  geom = sliceGeometry(geom, plane);
  scene.add(new THREE.Mesh(geom.toBufferGeometry(), material));

  var geom = new THREE.BoxGeometry(0.8, 0.8, 0.8);
  m.setPosition(new THREE.Vector3(1.2, -0.8, 0));
  geom.applyMatrix4(m);
  geom = sliceGeometry(new TGH.Gmetry().fromBufferGeometry(geom), plane);
  ...
  m.setPosition(new THREE.Vector3(1.2, 0.8, 0));
  geom.applyMatrix4(m);
  geom = sliceGeometry(new TGH.Gmetry().fromBufferGeometry(geom), plane, true);
  scene.add(new THREE.Mesh(geom.toBufferGeometry(), mapMaterial));
